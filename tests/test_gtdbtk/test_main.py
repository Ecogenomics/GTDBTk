...
class TestOptionsParser(unittest.TestCase):
    ...
    def test__verify_genome_id__valid(self):
        ...
        self.assertTrue(self.options_parser._assert_genome_id_valid('genome_1'))

    def test__verify_genome_id__invalid(self):
        """ Test that invalid genome ids throw an exception. """
        for c in list('()[],;='):
            self.assertRaises(GenomeNameInvalid, self.options_parser._assert_genome_id_valid, 'genome%s1' % c)
