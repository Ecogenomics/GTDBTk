...
class Decorate():
    ...
    def run(self, 
        ...
        if len(ambiguous_placements) > 0:
            self.logger.warning('There are {:,} taxa with multiple placements of equal quality.'.format(len(ambiguous_placements)))
            self.logger.warning('These were resolved by placing the label at the most terminal position.')
            self.logger.warning('Ideally, taxonomic assignment of all genomes should be establish before tree decoration.')
