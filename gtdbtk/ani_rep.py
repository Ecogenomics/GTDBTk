...
class ANIRep(object):
    ...
    def run(self, genomes, no_mash, mash_d, out_dir, prefix, mash_k, mash_v, mash_s, min_af, mash_db):
        ...
        skani_results,genome_size = self.run_mash_skani(genomes, no_mash, mash_d, out_dir,
    ...
    def run_mash_skani(self,genomes, no_mash, max_d, out_dir, prefix, mash_k, mash_v, mash_s, mash_max_dist=100, mash_db=None):
        ...
        skani_results,genome_size = skani.run(d_compare, d_paths)
        return skani_results,genome_size

class ANISummaryFile(object):
    name = 'ani_summary.tsv'

    def __init__(self, root=None, prefix=None, results=None,genome_size=None, taxonomy=None,marker_set_id=None):
    ...
    def get_col_order(ani_screen_step=False) -> List[str]:
        ...
        cols = ['user_genome',
                   ...
                   'skani_af',
                   'reference_taxonomy',
                'user_genome_size','reference_genome_size']
        if ani_screen_step:
        ...
        with open(self.path, 'w') as fh:
            ...
            for qry_gid, ref_hits in sorted(self.results.items()):
                for ref_gid, ref_hit in sorted(ref_hits.items(), key=lambda x: (-x[1]['af'], -x[1]['ani'], x[0])):
                    ...
                    fh.write(f'\t{self.genome_size[qry_gid]}\t{self.genome_size[ref_gid]}')
                    if ani_screen_step:
    ...
    def read(self):
        ...
        with open(self.path) as fh:
            ...
            for line in fh.readlines():
                qry_gid, ref_gid, ani, af, taxonomy_str,query_size,reference_size,other_refs = line.strip('\n').split('\t')
                results[qry_gid]={ref_gid:{'ani': float(ani), 'af': float(af), 'taxonomy': taxonomy_str,'other_refs':other_refs,'query_size':query_size,'reference_size':reference_size}}
        return results
